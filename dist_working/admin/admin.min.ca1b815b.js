...
}), angular.module("bahmni.common.config", []), angular.module("bahmni.common.config").directive("showIfPrivilege", ["$rootScope", function($rootScope) {
    return {
        scope: {
            showIfPrivilege: "@"
        },
        ...
        link: function($scope, element, attrs, ngModelCtrl) {
            $timeout(function() {
                ...
                ngModelCtrl.$formatters.push(function(modelValue) {
                    ...
                    return modelValue.replace(/^\d{2}(\d{2})/, "$1"); 
                });
            }, 400);
        }
    };
...
var Bahmni = Bahmni || {};
Bahmni.Admin = Bahmni.Admin || {}, angular.module("bahmni.admin", ["bahmni.common.uiHelper", "bahmni.common.domain", "bahmni.common.util", "bahmni.common.config", "bahmni.common.orders", "bahmni.common.appFramework", "bahmni.common.logging", "ui.router", "angularFileUpload"]), angular.module("bahmni.admin").factory("initialization", ["$rootScope", "$q", "appService", "spinner", function($rootScope, $q, appService, spinner) {
        ...
        moment.locale($window.localStorage.NG_TRANSLATE_LANG_KEY || "en"), $rootScope.$on("$viewContentLoaded", $templateCache.removeAll)
    }]), Bahmni.Admin.ImportedItem = function(data) {
        angular.extend(this, data), this.baseUrl = "/uploaded-files/mrs"
        ...
        var OrderSet = function(set) {
            ...
            OrderSetMember = function(member) {
                angular.extend(this, {
                    uuid: member.uuid,
                    orderType: {
            ...
            createOrderSetMember = function(orderSetMember) {
                var member = orderSetMember || {};
                return member.orderType = member.orderType || {}, member.concept = member.concept || {}, member.drug = member.drug || {}, member.dosingInstructions = member.dosingInstructions || {}, new OrderSetMember(member)
            },
            create = function(orderSet) {
                var set = orderSet || {};
                return set.orderSetMembers = _.map(set.orderSetMembers, createOrderSetMember), new OrderSet(set)
            };
        return {
            create: create,
            createOrderSetMember: createOrderSetMember
        }
    }(), angular.module("bahmni.admin").controller("CSVUploadController", ["$scope", "FileUploader", "appService", "adminImportService", "spinner", function($scope, FileUploader, appService, adminImportService, spinner) {
        ...
        var patientMatchingAlgorithm = adminCSVExtension.extensionParams.patientMatchingAlgorithm || "",
            urlMap = {
                concept: {
                    name: "Concept",
                    url: Bahmni.Common.Constants.conceptImportUrl
                },
                conceptset: {
                    name: "Concept Set",
                    url: Bahmni.Common.Constants.conceptSetImportUrl
                },
                program: {
                    name: "Program",
                    url: Bahmni.Common.Constants.programImportUrl
                },
                patient: {
                    name: "Patient",
                    url: Bahmni.Common.Constants.patientImportUrl
                },
                encounter: {
                    name: "Encounter",
                ...
                drug: {
                    name: "Drug",
                    url: Bahmni.Common.Constants.drugImportUrl
                },
                labResults: {
                    name: "Lab Results",
                    url: Bahmni.Common.Constants.labResultsImportUrl
                },
                referenceterms: {
                    name: "Reference Terms",
                    url: Bahmni.Common.Constants.referenceTermsImportUrl
                },
                relationship: {
                    name: "Relationship Information",
                    url: Bahmni.Common.Constants.relationshipImportUrl
                }
            },
            fileUploaderOptions = {
                removeAfterUpload: !0,
        ...
        var init = function() {
            ...
            _.isEmpty(configUrlMap) || (urlMap = configUrlMap), $scope.urlMaps = urlMap
        };
        $scope.option = {
            selected: "encounter"
        }, $scope.uploader = new FileUploader(fileUploaderOptions), $scope.uploader.onBeforeUploadItem = function(item) {
            item.url = urlMap[$scope.option.selected].url
        }, $scope.uploader.onCompleteAll = $scope.loadImportedItems, $scope.loadImportedItems(), init()
    }]), angular.module("bahmni.admin").controller("CSVExportController", ["$scope", "$state", "appService", "$http", function($scope, $state, appService, $http) {
        $scope.appExtensions = appService.getAppDescriptor().getExtensions("bahmni.admin.csvExport", "link") || [], $scope.conceptNameInvalid = !1, $scope.getConcepts = function(request) {
        ...
        }, $scope.conceptSet = null, $scope.getDataResults = function(results) {
            ...
            $scope.conceptSet && window.open(Bahmni.Common.Constants.conceptSetExportUrl.replace(":conceptName", $scope.conceptSet))
        }
    }]), angular.module("bahmni.admin").controller("auditLogController", ["$scope", "spinner", "auditLogService", "messagingService", "$translate", "appService", function($scope, spinner, auditLogService, messagingService, $translate, appService) {
        ...
        var defaultMessage = "",
            ...
            mapParamsForRequest = function(params) {
                return _.pickBy(params, isNotEmpty)
            },
            updateIndex = function(logs, defaultFirstIndex, defaultLastIndex) {
            ...
            setMessage = function(logsLength, message) {
                $scope.errorMessage = logsLength ? defaultMessage : message
            },
            updatePage = function(logs, defaultFirstIndex, defaultLastIndex, message) {
                logs.length && ($scope.logs = logs), setMessage(logs.length, message), updateIndex(logs, defaultFirstIndex, defaultLastIndex)
            },
            getDate = function() {
                var date = $scope.startDate || $scope.today;
                $scope.startDate = date;
                var dateBs = $scope.startDateBs || $scope.npToday;
                return $scope.startDateBs = dateBs, date
            },
            defaultView = function(params, message) {
                return auditLogService.getLogs(params).then(function(logs) {
                    logs.reverse(), updatePage(logs, 0, 0, message)
                })
            };
        $scope.next = function() {
            var params = {
            ...
            var promise, message = getTranslatedMessage("NO_MORE_EVENTS_FOUND");
            ...
            } else promise = defaultView(mapParamsForRequest({
                defaultView: !0,
                startFrom: $scope.startDate
            }), message);
            spinner.forPromise(promise)
        }, 
        // $scope.today = DateUtil.today(), $scope.maxDate = DateUtil.getDateWithoutTime($scope.today), $scope.updateAdDate = function(startDateBs) {
        ...
        $scope.updateAdDate = function(startDateBs) {
            var dateStr = startDateBs.split("-");
            var dateAD = NepaliFunctions.BS2AD({
                year: Number(NepaliFunctions.ConvertToNumber(dateStr[0])),
                month: Number(NepaliFunctions.ConvertToNumber(dateStr[1])),
                day: Number(NepaliFunctions.ConvertToNumber(dateStr[2]))
            });
            $scope.startDate = new Date(dateAD.year, dateAD.month - 1, dateAD.day);
        };
        $scope.runReport = function() {
            if ($("#startDate").hasClass("ng-invalid-max")) return void messagingService.showMessage("error", getTranslatedMessage("INVALID_DATE"));
            var params = {
                    username: $scope.username,
                    patientId: $scope.patientId,
                    startFrom: $scope.startDate
                ...
                promise = auditLogService.getLogs(mapParamsForRequest(params)).then(function(logs) {
                    $scope.logs = logs, setMessage(logs.length, getTranslatedMessage("MATCHING_EVENTS_NOT_FOUND")), updateIndex(logs, 0, 0)
                });
            spinner.forPromise(promise)
        };
        var init = function() {
            $scope.logs = [];
            var promise = defaultView({
                startFrom: getDate(),
                defaultView: !0
            }, getTranslatedMessage("NO_EVENTS_FOUND"));
            spinner.forPromise(promise)
        };
        init()
    }]), angular.module("bahmni.common.domain").controller("OrderSetDashboardController", ["$scope", "$state", "spinner", "appService", "$http", "adminOrderSetService", "$location", function($scope, $state, spinner, appService, $http, adminOrderSetService, $location) {
        $scope.appExtensions = appService.getAppDescriptor().getExtensions("bahmni.admin.orderSet", "link") || [], $scope.createOrEditOrderSet = function(uuid) {
            uuid || (uuid = "new");
            var url = "/orderset/" + uuid;
            $location.url(url)
        }, $scope.removeOrderSet = function(orderSet) {
            var orderSetObj = Bahmni.Common.OrderSet.create(orderSet);
            orderSetObj.retired = !0, spinner.forPromise(adminOrderSetService.removeOrderSet(orderSetObj)).then(function(response) {
                init()
            })
        };
        var init = function() {
    ...
    function() {
        var mapResponse = function(concept) {
            ...
            updateOrderSetMemberConcept = function(newOrderSetMember, oldOrderSetMember) {
                oldOrderSetMember.concept.display = newOrderSetMember.concept.display, oldOrderSetMember.concept.uuid = newOrderSetMember.concept.uuid
            },
            moveUp = function(array, element) {
                var index = _.indexOf(array, element),
                    firstIndex = 0;
                return index !== firstIndex && (array.splice(index, 1), array.splice(index - 1, 0, element), !0)
            },
            moveDown = function(array, element) {
                var index = _.indexOf(array, element),
                    lastIndex = array.length - 1;
                return index !== lastIndex && (array.splice(index, 1), array.splice(index + 1, 0, element), !0)
            };
        angular.module("bahmni.common.domain").controller("OrderSetController", ["$scope", "$state", "spinner", "$http", "$q", "adminOrderSetService", "messagingService", "orderTypeService", "$window", function($scope, $state, spinner, $http, $q, adminOrderSetService, messagingService, orderTypeService, $window) {
            $scope.operators = ["ALL", "ANY", "ONE"], $scope.conceptNameInvalid = !1, $scope.addOrderSetMembers = function() {
                $scope.orderSet.orderSetMembers.push(buildOrderSetMember())
            };
            var isOrderSetHavingMinimumOrders = function() {
                return _.filter($scope.orderSet.orderSetMembers, function(setMember) {
                    return !setMember.retired
                }).length >= 1
            };
            $scope.remove = function(orderSetMember) {
                0 == orderSetMember.retired ? orderSetMember.retired = !0 : _.remove($scope.orderSet.orderSetMembers, orderSetMember)
            ...
            var getConcepts = function(request, isOrderTypeMatching) {
                return $http.get(Bahmni.Common.Constants.conceptUrl, {
                    ...
                    var results = _.get(response, "data.results"),
                        resultsMatched = _.filter(results, isOrderTypeMatching);
            ...
            $scope.getConcepts = function(orderSetMember) {
                    var selectedOrderType = orderSetMember.orderType,
                        orderType = _.find($scope.orderTypes, {
                            uuid: selectedOrderType.uuid
                        }),
                        orderTypeNames = _.map(orderType.conceptClasses, "name"),
                        isOrderTypeMatching = function(concept) {
                ...
                function() {
                    ...
                    $scope.onSelect = function(oldOrderSetMember) {
                        ...
                        _.isUndefined(currentOrderSetMember) || (currentOrderSetMember.concept.uuid = oldOrderSetMember.concept.uuid, newOrderSetMember = null)
                    }, $scope.onChange = function(oldOrderSetMember) {
                        return newOrderSetMember ? (updateOrderSetMemberConcept(newOrderSetMember, oldOrderSetMember), void(newOrderSetMember = null)) : (oldOrderSetMember.orderTemplate = {}, void delete oldOrderSetMember.concept.uuid)
                    }
                }(), $scope.clearConceptName = function(orderSetMember) {
                    orderSetMember.concept = {}, orderSetMember.orderTemplate = {}
                }, $scope.save = function() {
                    return validationSuccess() ? (getValidOrderSetMembers(), spinner.forPromise(adminOrderSetService.createOrUpdateOrderSet($scope.orderSet).then(function(response) {
                        return $state.params.orderSetUuid = response.data.uuid, $state.transitionTo($state.current, $state.params, {
                            reload: !0,
                            inherit: !1,
                            notify: !0
                        }).then(function() {
            ...
            var getValidOrderSetMembers = function() {
                ...
                validationSuccess = function() {
                    return !!validateForm() && (!(!$scope.orderSet.orderSetMembers || !isOrderSetHavingMinimumOrders()) || (messagingService.showMessage("error", "An orderSet should have a minimum of two orderSetMembers"), !1))
                },
                buildOrderSetMember = function() {
                    return {
                        orderType: {
                            uuid: $scope.orderTypes[0].uuid
                        }
                    }
                },
                validateForm = function() {
                    for (var requiredFields = angular.element($("[required]")), i = 0; i < requiredFields.length; i++)
                        if (!requiredFields[i].disabled && !requiredFields[i].value) return messagingService.showMessage("error", "Please fill all mandatory fields"), !1;
                    return !0
                },
                init = function() {
                    var init = $q.all([orderTypeService.loadAll(), adminOrderSetService.getDrugConfig()]).then(function(results) {
                        $scope.orderTypes = results[0], $scope.treatmentConfig = results[1], "new" !== $state.params.orderSetUuid ? spinner.forPromise(adminOrderSetService.getOrderSet($state.params.orderSetUuid).then(function(response) {
                            $scope.orderSet = Bahmni.Common.OrderSet.create(response.data)
                        })) : ($scope.orderSet = Bahmni.Common.OrderSet.create(), $scope.orderSet.operator = $scope.operators[0], $scope.orderSet.orderSetMembers.push(Bahmni.Common.OrderSet.createOrderSetMember(buildOrderSetMember()), Bahmni.Common.OrderSet.createOrderSetMember(buildOrderSetMember())))
                    });
    ...
    function() {
        var mapResult = function(drug) {
            ...
            selectDrug = function(selectedTemplate, orderSetMember) {
                orderSetMember.orderTemplate.drug = selectedTemplate.drug
            },
            deleteDrugIfDrugNameIsEmpty = function(orderSetMember) {
                orderSetMember.orderTemplate.drug.name || (orderSetMember.orderTemplate.drug = {})
            },
            $inject = ["$scope", "drugService"],
            OrderTemplateController = function($scope, drugService) {
                var search = function(request, orderSetMember) {
                    return drugService.search(request.term, orderSetMember.concept.uuid).then(_.partial(_.map, _, mapResult))
                };
                ...
                }, $scope.onChange = deleteDrugIfDrugNameIsEmpty, $scope.isRuleMode = function(orderSetMember) {
                    return "undefined" != typeof orderSetMember.orderTemplate.dosingInstructions && null != orderSetMember.orderTemplate.dosingInstructions.dosingRule
                }
            };
        OrderTemplateController.$inject = $inject, angular.module("bahmni.common.domain").controller("OrderTemplateController", OrderTemplateController)
    }(), angular.module("bahmni.admin").controller("AdminDashboardController", ["$scope", "$state", "appService", function($scope, $state, appService) {
        $scope.appExtensions = appService.getAppDescriptor().getExtensions($state.current.data.extensionPointId, "link") || []
    }]), angular.module("bahmni.admin").service("adminImportService", ["$http", function($http) {
        ...
        this.getAllOrderSets = function() {
            ...
            var url;
            return _.each(orderSet.orderSetMembers, function(orderSetMember) {
                orderSetMember.orderTemplate && (orderSetMember.orderTemplate = JSON.stringify(orderSetMember.orderTemplate))
            }), url = orderSet.uuid ? Bahmni.Common.Constants.orderSetUrl + "/" + orderSet.uuid : Bahmni.Common.Constants.orderSetUrl, $http.post(url, orderSet, {
                withCredentials: !0,
                headers: {
    ...
    }]), angular.module("bahmni.common.orders", []), angular.module("bahmni.common.orders").service("orderTypeService", ["$http", function($http) {
        var self = this;
