...
Bahmni.Common = Bahmni.Common || {},
    function() {
        ...
        this.getPatient = function(uuid) {
            ...
            return offset = offset || 0, $http.get(Bahmni.Common.Constants.bahmniSearchUrl + "/patient", {
                method: "GET",
                params: {
                    q: query,
                    startIndex: offset,
...
}, angular.module("bahmni.common.uiHelper", ["ngClipboard"]), angular.module("bahmni.common.uiHelper").filter("reverse", function() {
...
.filter("npDate", function() {
    return function(date) {
        if (date != null && date !== undefined && date !== "" && Bahmni.Common.Util.DateUtil.isValid(date)) {
            ...
            return `${NepaliFunctions.ConvertToUnicode(bsDate.year)} ${monthName}, ${NepaliFunctions.ConvertToUnicode(bsDate.day)}`; // "२०८२ बैशाख, ३१"
        }
        return date;
    };
})
...
.filter("npDateTime", function() {
    return function(date) {
        if (date != null && date !== undefined && date !== "" && Bahmni.Common.Util.DateUtil.isValid(date)) {
            ...
            var bsDate = NepaliFunctions.AD2BS(adDateObj); 
            var monthName = NepaliFunctions.BS.GetMonthInUnicode(bsDate.month - 1);
            return `${NepaliFunctions.ConvertToUnicode(bsDate.year)} ${monthName}, ${NepaliFunctions.ConvertToUnicode(bsDate.day)}`; // "२०८२ बैशाख, १५"
        }
        return date;
    };
}),
angular.module("bahmni.common.uiHelper").factory("spinner", ["messagingService", "$timeout", function(messagingService, $timeout) {
    ...
    //CHANGED BY BHAWANA
    getDate: function(dateTime) {
        console.log("getDate input:", dateTime, typeof dateTime);
        if (!dateTime) {
            console.warn("dateTime is undefined or null");
            return null;
        }
        try {
            var adDate = this.parse(dateTime);
            if (!(adDate instanceof Date) || isNaN(adDate)) {
                console.warn("Invalid dateTime after parsing:", dateTime);
                return null;
            }
            ...
            if (NepaliFunctions.BS.ValidateDate(bsDate)) {
                console.log("BS equivalent:", `${bsDate.year}-${bsDate.month}-${bsDate.day}`);
            } else {
                console.warn("Invalid BS date:", bsDate);
            }
            return adDate;
        } catch (e) {
            console.error("Error processing dateTime:", dateTime, e);
            return null;
        }
    },
...
//                         var bsDate = date.bs; 
//                         var formattedDate = bsDate.replace(/^\d{2}(\d{2})/, "$1");
//                         $scope.$apply(function() {
...
//                 if (attrs.min && attrs.min !== "undefined") {
//                     var minYear = parseInt(attrs.min.split("-")[0]) + 2000; 
//                     options.disableBefore = `${minYear}-${attrs.min.slice(3)}`; 
//                 }

//                 if (attrs.max && attrs.max !== "undefined") {
//                     var maxYear = parseInt(attrs.max.split("-")[0]) + 2000;
//                     options.disableAfter = `${maxYear}-${attrs.max.slice(3)}`; 
//                 }
//                     if (attrs.allowFutureDates) {
//                     options.disableAfter = null;
//                 }
...
//                     var year = parseInt(viewValue.split("-")[0]);
//                     var fullYear = year < 100 ? year + 2000 : year; 
//                     var formatted = `${fullYear}-${viewValue.slice(3)}`;
//                     var dateObj = NepaliFunctions.ConvertToDateObject(formatted, "YYYY-MM-DD");
//                     return NepaliFunctions.BS.ValidateDate(dateObj) ? formatted : undefined;
//                 });
...
angular.module("bahmni.common.uiHelper").directive("npdatepicker", ["$timeout", function($timeout) {
    return {
        ...
        link: function($scope, element, attrs, ngModelCtrl) {
            $timeout(function() {
                // Calculate current BS date as maxDate
                var todayAd = new Date();
                var todayBs = NepaliFunctions.AD2BS({
                    year: todayAd.getFullYear(),
                    month: todayAd.getMonth() + 1,
                    day: todayAd.getDate()
                });
                var defaultMaxDate = `${todayBs.year}-${NepaliFunctions.Get2DigitNo(todayBs.month)}-${NepaliFunctions.Get2DigitNo(todayBs.day)}`; // e.g., "2082-01-07"

                // Initialize datepicker options
                var options = {
                    dateFormat: "YYYY-MM-DD", // Internal BS date format
                    closeOnDateSelect: true,
                    unicodeDate: false, // Standard digits
                    onChange: function(date) {
                        console.log("Datepicker selected:", date.bs);
                        if (!date.bs) {
                            $scope.$apply(function() {
                        ...
                        var dateParts = date.bs.split("-");
                        var year = parseInt(dateParts[0]);
                        var month = parseInt(dateParts[1]);
                        var day = parseInt(dateParts[2]);
                        if (NepaliFunctions.BS.ValidateDate({ year, month, day })) {
                            var monthName = NepaliFunctions.BS.GetMonthInUnicode(month - 1);
                            var formattedDate = `${NepaliFunctions.ConvertToUnicode(year)} ${monthName}, ${NepaliFunctions.ConvertToUnicode(day)}`; // e.g., "२०८२ बैशाख, १५"
                            $scope.$apply(function() {
                                ngModelCtrl.$setViewValue(formattedDate);
                                ngModelCtrl.$render();
                            });
                        } else {
                            console.warn("Invalid BS date:", date.bs);
                            $scope.$apply(function() {
                                ngModelCtrl.$setViewValue("");
                                ngModelCtrl.$render();
                            });
                ...
                // Handle minDate
                if (attrs.min && attrs.min !== "undefined" && attrs.min !== "") {
                    var minYear = parseInt(attrs.min.split("-")[0]);
                    minYear = minYear < 100 ? minYear + 2000 : minYear; // Convert "82" to 2082
                    options.disableBefore = `${minYear}-${NepaliFunctions.Get2DigitNo(parseInt(attrs.min.split("-")[1]))}-${NepaliFunctions.Get2DigitNo(parseInt(attrs.min.split("-")[2]))}`;
                }

                // Handle maxDate
                if (attrs.max && attrs.max !== "undefined" && attrs.max !== "") {
                    var maxYear = parseInt(attrs.max.split("-")[0]);
                    maxYear = maxYear < 100 ? maxYear + 2000 : maxYear;
                    options.disableAfter = `${maxYear}-${NepaliFunctions.Get2DigitNo(parseInt(attrs.max.split("-")[1]))}-${NepaliFunctions.Get2DigitNo(parseInt(attrs.max.split("-")[2]))}`;
                } else {
                    // Default to current BS date to prevent future dates
                    options.disableAfter = defaultMaxDate;
                }

                // Allow future dates only if explicitly specified
                if (attrs.allowFutureDates) {
                    options.disableAfter = null;
                }
                ...
                // View to Model: Convert BS date to AD Date
                ngModelCtrl.$parsers.push(function(viewValue) {
                    console.log("Parser input:", viewValue);
                    if (!viewValue || typeof viewValue !== 'string') {
                        return undefined;
                    }
                    var parts = viewValue.split(/[\s,]+/);
                    if (parts.length !== 3) {
                        console.warn("Invalid date format:", viewValue);
                        return undefined;
                    }
                    var year = Number(NepaliFunctions.ConvertToNumber(parts[0]));
                    var monthName = parts[1];
                    var day = Number(NepaliFunctions.ConvertToNumber(parts[2]));
                    var month = -1;
                    for (var i = 0; i < 12; i++) {
                        if (NepaliFunctions.BS.GetMonthInUnicode(i) === monthName) {
                            month = i + 1;
                            break;
                        }
                    }
                    if (month === -1 || !NepaliFunctions.BS.ValidateDate({ year, month, day })) {
                        console.warn("Invalid BS date:", { year, month, day });
                        return undefined;
                    }
                    var adDate = NepaliFunctions.BS2AD({ year, month, day });
                    if (adDate) {
                ...
                // Model to View: Convert AD Date to BS date
                ngModelCtrl.$formatters.push(function(modelValue) {
                    console.log("Formatter input:", modelValue);
                    if (!modelValue || !(modelValue instanceof Date) || isNaN(modelValue)) {
                        return "";
                    }
                    var adDateObj = {
                        year: modelValue.getFullYear(),
                        month: modelValue.getMonth() + 1,
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.I18n = Bahmni.Common.I18n || {}, angular.module("bahmni.common.i18n", []), angular.module("bahmni.common.i18n", ["pascalprecht.translate"]).provider("$bahmniTranslate", ["$translateProvider", function($translateProvider) {
    ...
    FastClick.attach(document.body), moment.locale($window.localStorage.NG_TRANSLATE_LANG_KEY || "en"), backlinkService.addBackUrl()
}]);
var Bahmni = Bahmni || {};
Bahmni.DocumentUpload = Bahmni.DocumentUpload || {}, angular.module("opd.documentupload", ["bahmni.common.patient", "bahmni.common.config", "bahmni.common.domain", "bahmni.common.gallery", "bahmni.common.logging"]), Bahmni.Common.AuditLogEventDetails = {
    USER_LOGIN_SUCCESS: {
...
}, angular.module("opd.documentupload").factory("initialization", ["$rootScope", "$q", "$window", "$location", "configurationService", "configurations", "authenticator", "appService", "spinner", function($rootScope, $q, $window, $location, configurationService, configurations, authenticator, appService, spinner) {
    var initializationPromise = $q.defer(),
        url = purl(decodeURIComponent($window.location));
    ...
    var getConfigs = function() {
        ...
        getConsultationConfigs = function() {
        ...
        validate = function() {
            var deferrable = $q.defer(),
                throwValidationError = function(errorMessage) {
                    $rootScope.error = errorMessage, initializationPromise.reject(), deferrable.reject()
                };
            return null === $rootScope.appConfig.encounterType ? throwValidationError("encounterType should be configured in config") : null === $rootScope.encounterConfig.getEncounterTypeUuid($rootScope.appConfig.encounterType) && throwValidationError("Configured encounterType does not exist"), deferrable.resolve(), deferrable
        },
        ...
        initApp = function() {
            return appService.initApp("documentUpload", {
                app: !0,
                extension: !0
    ...
    }), spinner.forPromise(initializationPromise.promise).then(getConfigs)
}]);
var Bahmni = Bahmni || {};
Bahmni.DocumentUpload = Bahmni.DocumentUpload || {}, Bahmni.DocumentUpload.Constants = function() {
    return {
    ...
    this.startDatetime = "", this.stopDatetime = "", this.visitType = null, this.uuid = null, this.changed = !1, this.files = [];
    var androidDateFormat = "YYYY-MM-DD hh:mm:ss";
    this._sortSavedFiles = function(savedFiles) {
        return savedFiles.sort(function(file1, file2) {
        ...
        }), this.files = this._sortSavedFiles(savedFiles), this.assignImageIndex()
    }, this.assignImageIndex = function() {
        var imageIndex = this.getNoOfImages() - 1;
        this.files.map(function(file) {
            return file.encodedValue.indexOf(".pdf") > 0 || (file.imageIndex = imageIndex, imageIndex--), file
        })
     ...
     this.startDate = function() {
        return this.isNew() ? this.parseDate(this.startDatetime) : moment(this.startDatetime).toDate()
    },
     this.endDate = function() {
        return this.stopDatetime ? this.parseDate(this.stopDatetime) : void 0
    },
     this.parseDate = function(date) {
        ...
        var savedImage = null,
            alreadyPresent = this.files.filter(function(img) {
                return img.encodedValue === file
            });
        return 0 === alreadyPresent.length && (savedImage = new DocumentImage({
            encodedValue: file,
            "new": !0
        }), this.files.push(savedImage)), this.assignImageIndex(), this.markAsUpdated(), savedImage
    }, this.markAsUpdated = function() {
        this.changed = this.files.some(function(file) {
            return file.changed || !file.obsUuid || file.voided
        })
    }, this.isSaved = function(file) {
        return !!file.obsUuid
    }, this.removeFile = function(file) {
        this.isSaved(file) ? this.toggleVoidingOfFile(file) : this.removeNewAddedFile(file)
    }, this.removeNewAddedFile = function(file) {
        var i = this.files.indexOf(file);
        this.files.splice(i, 1), this.assignImageIndex(), this.markAsUpdated()
    }, this.toggleVoidingOfFile = function(file) {
        file.voided = !file.voided, this.markAsUpdated()
...
}, angular.module("opd.documentupload").directive("fileUpload", [function() {
    var link = function(scope, element) {
...
}]), angular.module("opd.documentupload").directive("dateValidator", function() {
    var DateUtil = Bahmni.Common.Util.DateUtil,
        isVisitDateFromFuture = function(visitDate) {
            return !(!visitDate.startDatetime && !visitDate.stopDatetime) && (DateUtil.getDate(visitDate.startDatetime) > new Date || DateUtil.getDate(visitDate.stopDatetime) > new Date)
        },
        isStartDateBeforeEndDate = function(visitDate) {
            return !visitDate.startDatetime || !visitDate.stopDatetime || DateUtil.getDate(visitDate.startDatetime) <= DateUtil.getDate(visitDate.stopDatetime)
        };
    return {
        restrict: "A",
        require: "ngModel",
        link: function(scope, element, attrs, ngModel) {
            function validate() {
                ngModel.$setValidity("overlap", scope.isNewVisitDateValid()), ngModel.$setValidity("future", !isVisitDateFromFuture(scope.newVisit)), ngModel.$setValidity("dateSequence", isStartDateBeforeEndDate(scope.newVisit))
            }
            scope.$watch(attrs.ngModel, validate), scope.$watch(attrs.dependentModel, validate)
        }
    }
}), angular.module("opd.documentupload").controller("DocumentController", ["$scope", "$stateParams", "visitService", "patientService", "encounterService", "spinner", "visitDocumentService", "$rootScope", "$http", "$q", "$timeout", "sessionService", "$anchorScroll", "$translate", "messagingService", "appService", function($scope, $stateParams, visitService, patientService, encounterService, spinner, visitDocumentService, $rootScope, $http, $q, $timeout, sessionService, $anchorScroll, $translate, messagingService, appService) {
    $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
    var encounterTypeUuid, topLevelConceptUuid, customVisitParams = Bahmni.DocumentUpload.Constants.visitRepresentation,
        DateUtil = Bahmni.Common.Util.DateUtil,
        patientMapper = new Bahmni.PatientMapper($rootScope.patientConfig, $rootScope, $translate),
        activeEncounter = {},
        locationUuid = sessionService.getLoginLocationUuid();
    $scope.visits = [], $scope.fileTypeConcepts = [], $scope.toggleGallery = !0, $scope.conceptNameInvalid = !1;
    ...
    window.addEventListener("orientationchange", onOrientationChange), $scope.$on("$destroy", function() {
        window.removeEventListener("orientationchange", onOrientationChange)
    });
    var initNewVisit = function() {
            $scope.newVisit = new Bahmni.DocumentUpload.Visit, $scope.currentVisit = $scope.newVisit
        },
        ...
        getVisitStartStopDateTime = function(visit) {
            return {
                startDatetime: DateUtil.getDate(visit.startDatetime),
                stopDatetime: DateUtil.getDate(visit.stopDatetime)
            }
        },
        isVisitInSameRange = function(newVisitWithoutTime, existingVisit) {
    ...
    $scope.isNewVisitDateValid = function() {
        var filterExistingVisitsInSameDateRange = function(existingVisit) {
                return !DateUtil.isInvalid(newVisitWithoutTime.startDatetime) && isVisitInSameRange(newVisitWithoutTime, existingVisit)
            },
            newVisitWithoutTime = {};
        // newVisitWithoutTime.startDatetime = DateUtil.getDate($scope.newVisit.startDatetime), newVisitWithoutTime.stopDatetime = $scope.newVisit.stopDatetime ? DateUtil.getDate($scope.newVisit.stopDatetime) : DateUtil.now();
        
        //CHANGED BY BHAWANA
        newVisitWithoutTime.startDatetime = DateUtil.npToday($scope.newVisit.startDatetime), newVisitWithoutTime.stopDatetime = $scope.newVisit.stopDatetime ? DateUtil.npToday($scope.newVisit.stopDatetime) : DateUtil.now();
        var visitStartStopDateTime = $scope.visits.map(getVisitStartStopDateTime),
            existingVisitsInSameRange = visitStartStopDateTime.filter(filterExistingVisitsInSameDateRange);
        return $scope.isDateValid = 0 === existingVisitsInSameRange.length, 0 === existingVisitsInSameRange.length
    };
    var getVisits = function() {
            return visitService.search({
                ...
                visits.length > 0 && (visits[0].stopDatetime ? $scope.currentVisit = null : $scope.currentVisit = visits[0]), visits.forEach(function(visit) {
                    $scope.visits.push(createVisit(visit))
                })
            })
        },
        getEncountersForVisits = function() {
        ...
        getTopLevelConcept = function() {
            return null === $rootScope.appConfig.topLevelConcept ? (topLevelConceptUuid = null, $q.when({})) : $http.get(Bahmni.Common.Constants.conceptSearchByFullNameUrl, {
                params: {
                    name: $rootScope.appConfig.topLevelConcept,
                    v: "custom:(uuid,setMembers:(uuid,name:(name)))"
                }
            }).then(function(response) {
                response.data.results[0].setMembers && response.data.results[0].setMembers.length > 0 && response.data.results[0].setMembers.forEach(function(concept) {
                    var conceptToAdd = {
                        concept: {
                            uuid: concept.uuid,
        ...
        sortVisits = function() {
            $scope.visits.sort(function(a, b) {
                var date1 = DateUtil.parse(a.startDatetime),
                    date2 = DateUtil.parse(b.startDatetime);
        ...
        getActiveEncounter = function() {
            ...
            return encounterService.find({
                ...
                providerUuids: _.isEmpty(currentProviderUuid) ? null : [currentProviderUuid],
                includeAll: Bahmni.Common.Constants.includeAllObservations,
        ...
        init = function() {
            encounterTypeUuid = $scope.encounterConfig.getEncounterTypeUuid($rootScope.appConfig.encounterType), initNewVisit();
            var deferrables = $q.defer(),
                promises = [];
            return promises.push(getVisitTypes()), promises.push(getActiveEncounter()), promises.push(getPatient().then(getVisits).then(getEncountersForVisits)), promises.push(getTopLevelConcept()), $q.all(promises).then(function() {
                deferrables.resolve()
            }), deferrables.promise
        };
    spinner.forPromise(init()), $scope.getConcepts = function(request) {
        ...
        $scope.toggleGallery = !1, fileType = visitDocumentService.getFileType(fileType), "not_supported" !== fileType ? spinner.forPromise(visitDocumentService.saveFile(file, $rootScope.patient.uuid, $rootScope.appConfig.encounterType, fileName, fileType).then(function(response) {
            var fileUrl = Bahmni.Common.Constants.documentsPath + "/" + response.data.url;
            visit.addFile(fileUrl);
            $scope.toggleGallery = !0
        }, function() {
            messagingService.showMessage("error"), $scope.toggleGallery = !0
        })) : (messagingService.showMessage("error", "File type is not supported"), $scope.toggleGallery = !0, $scope.$$phase || $scope.$apply())
    }, $scope.toInitFileConcept = function(file) {
        file.concept && file.concept.editableName || (file.concept = Object.create($scope.fileTypeConcepts[0].concept), file.changed = !0)
        ...
        selectedItem && selectedItem.concept && (file.concept = Object.create(selectedItem.concept)), file.changed = !0
    }, $scope.enableSaveButtonOnCommentChange = function(file, visit) {
        _.set(file, "changed", !0), _.set(visit, "changed", !0)
    }, $scope.resetCurrentVisit = function(visit) {
        return areVisitsSame($scope.currentVisit, visit) && areVisitsSame($scope.currentVisit, $scope.newVisit) ? ($scope.currentVisit = null, $scope.currentVisit) : void($scope.currentVisit = $scope.isCurrentVisit(visit) ? $scope.newVisit : visit)
    ...
    var areVisitsSame = function(currentVisit, newVisit) {
        ...
        flashSuccessMessage = function() {
            $scope.success = !0, $timeout(function() {
                $scope.success = !1
            }, 2e3)
        };
    $scope.setDefaultEndDate = function(newVisit) {
        newVisit.stopDatetime || ($scope.newVisit.stopDatetime = newVisit.endDate() ? DateUtil.parse(newVisit.endDate()) : new Date)
    };
    ...
    var updateVisit = function(visit, encounters) {
            ...
            visit.initSavedFiles(visitEncounters), visit.changed = !1, $scope.currentVisit = visit, sortVisits(), flashSuccessMessage()
        },
        isExistingVisit = function(visit) {
            return !!visit.uuid
        };
    $scope.save = function(visit) {
        $scope.toggleGallery = !1;
        var visitDocument;
        return (isExistingVisit(visit) || $scope.isNewVisitDateValid()) && (visitDocument = createVisitDocument(visit)), spinner.forPromise(visitDocumentService.save(visitDocument).then(function(response) {
            return encounterService.getEncountersForEncounterType($scope.patient.uuid, encounterTypeUuid).then(function(encounterResponse) {
                ...
                savedVisit ? (updateVisit(savedVisit, encounterResponse.data.results), $scope.toggleGallery = !0) : visitService.getVisit(response.data.visitUuid, customVisitParams).then(function(visitResponse) {
                    var newVisit = createVisit(visitResponse.data);
                    visit = $scope.visits.push(newVisit), initNewVisit(), updateVisit(newVisit, encounterResponse.data.results), $scope.toggleGallery = !0
                }), getActiveEncounter()
            })
        }))
