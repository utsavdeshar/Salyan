...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.Models = Bahmni.Common.Models || {}, angular.module("bahmni.common.models", []), angular.module("bahmni.common.models").factory("age", [function() {
    ...
    //         currentNepaliDate = calendarFunctions.getBsDateByAdDate(currentDate.getFullYear(), currentDate.getMonth() + 1, currentDate.getDate()-1);
...
! function() {
    ...
    //   if(bsDate.bsDate >= '19' && bsDate.bsMonth=='12')
    //     {
    //     ob1= calendarFunctions.bsDateFormat("%y-%m-%d",bsDate.bsYear, bsDate.bsMonth,bsDate.bsDate-1);
               
        
    //   }
    //   else{
    //     ob1 = calendarFunctions.bsDateFormat("%y %M, %d", bsDate.bsYear, bsDate.bsMonth, bsDate.bsDate);
    //   }
    ...
    //                 return ob1 + " " + Bahmni.Common.Util.DateUtil.formatTime(date)
        ...
        return function(date) {
            if (date != null && date !== undefined && date !== "" && Bahmni.Common.Util.DateUtil.isValid(date)) {
                try {
                    date = isNaN(Number(date)) ? date : Number(date);
                    var adDate = Bahmni.Common.Util.DateUtil.getDateWithoutTime(date).split("-");
                    if (adDate.length !== 3 || adDate.some(part => !part || isNaN(parseInt(part, 10)))) {
                        console.warn("npDateTime - Invalid AD date parts:", adDate);
                        return date;
                    }
                    var adDateObj = {
                        year: parseInt(adDate[0], 10),
                        month: parseInt(adDate[1], 10),
                        day: parseInt(adDate[2], 10)
                    };
                    var bsDate = NepaliFunctions.AD2BS(adDateObj);
                    if (!bsDate || !NepaliFunctions.BS.ValidateDate(bsDate)) {
                        console.warn("npDateTime - Invalid BS date:", bsDate);
                        return date;
                    }
                    var monthName = NepaliFunctions.BS.GetMonthInUnicode(bsDate.month - 1);
                    if (!monthName) {
                        console.warn("npDateTime - Invalid month name for index:", bsDate.month - 1);
                        return date;
                    }
                    return `${NepaliFunctions.ConvertToUnicode(bsDate.year)} ${monthName}, ${NepaliFunctions.ConvertToUnicode(bsDate.day)}`;
                } catch (e) {
                    console.error("npDateTime - Error formatting date:", e);
                    return date;
                }
            }
            return date || "";
                ...
                function updateDate(test) {
                    if ($scope.observation.label == "Expected delivery date") {
                        ...
                        if (result instanceof Date && !isNaN(result)) {
                            str = result.toISOString().substr(0, 10); // e.g., "2025-04-13"
                        }
                        test.englishDate = str;
                        $scope.observation.value = str;
                        var nepaliDate = "";
                        if (str) {
                            var date = str.split("-");
                            if (date && Array.isArray(date) && date.length === 3 && date.every(part => part && !isNaN(parseInt(part)))) {
                                try {
                                    var adDateObj = {
                                    ...
                                    if (!isNaN(adDateObj.year) && !isNaN(adDateObj.month) && !isNaN(adDateObj.day)) {
                                        var bsDate = NepaliFunctions.AD2BS(adDateObj);
                                        if (NepaliFunctions.BS.ValidateDate(bsDate)) {
                                            nepaliDate = NepaliFunctions.ConvertToDateFormat(bsDate, "YYYY-MM-DD"); // e.g., "2082-01-02"
                                        }
                                    }
                                } catch (e) {
...
//                 if(calendarFunctions.getNumberByNepaliNumber(obsBsDate.split('-')[2]) >= '19' && (calendarFunctions.getNumberByNepaliNumber(obsBsDate.split('-')[1]))=='12')
//                 {
//                     ob1= calendarFunctions.bsDateFormat("%y-%m-%d",calendarFunctions.getNumberByNepaliNumber(obsBsDate.split('-')[0]), 
//                     calendarFunctions.getNumberByNepaliNumber(obsBsDate.split('-')[1]), calendarFunctions.getNumberByNepaliNumber(obsBsDate.split('-')[2])-1);
...
angular.module("bahmni.common.conceptSet").controller("nepaliDateFieldObservationController", ["$rootScope", "$scope", "conceptSetService", function($rootScope, $scope, conceptSetService) {
    ...
    $scope.handleNepaliDateUpdate = function(observation) {
        if (observation && typeof observation.nepaliDate === "string" && observation.nepaliDate.trim()) {
            var date = dateUpdateCommon(observation);
            if (date instanceof Date && !isNaN(date)) {
                var month = date.getMonth() + 1;
                month = month < 10 ? "0" + month : month;
                var day = date.getDate() < 10 ? "0" + date.getDate() : date.getDate();
                observation.value = `${date.getFullYear()}-${month}-${day}`;
    ...
    var dateUpdateCommon = function(observation) {
        if (observation && typeof observation.nepaliDate === "string" && observation.nepaliDate.trim()) {
            try {
                var dateStr = NepaliFunctions.ConvertToNumber(observation.nepaliDate);
                var parts = dateStr.split("-");
                if (parts.length === 3 && parts.every(part => part.trim())) {
                    var year = parseInt(parts[0]);
                    if (isNaN(year)) return null;
                    if (year < 100) {
                        year += 2000; // Convert "82" to 2082
                        dateStr = `${year}-${parts[1]}-${parts[2]}`;
                    }
                    var bsDateObj = NepaliFunctions.ConvertToDateObject(dateStr, "YYYY-MM-DD");
                    if (NepaliFunctions.BS.ValidateDate(bsDateObj)) {
                        var adDateObj = NepaliFunctions.BS2AD(bsDateObj);
                        return new Date(adDateObj.year, adDateObj.month - 1, adDateObj.day);
                    }
                }
            } catch (e) {
    ...
    $scope.initializeNepaliDate = function(observation) {
        if (observation) {
            observation.allowFutureDate = observation.conceptUIConfig && observation.conceptUIConfig.allowFutureDates || false;
            if (typeof observation.value === "string" && observation.value.trim()) {
                try {
                    var date = observation.value.split("-");
                    if (date.length === 3 && date.every(part => part.trim())) {
                        var adDateObj = {
                            year: parseInt(date[0]),
                            month: parseInt(date[1]),
                            day: parseInt(date[2])
                        ...
                        if (!isNaN(adDateObj.year) && !isNaN(adDateObj.month) && !isNaN(adDateObj.day)) {
                            ...
                            if (NepaliFunctions.BS.ValidateDate(bsDate)) {
                                ...
                                observation.nepaliDate = formatted;
                            } else {
                                observation.nepaliDate = "";
                            }
                        } else {
                            observation.nepaliDate = "";
                        }
                    ...
                    console.error("Error initializing BS date:", e);
                    observation.nepaliDate = "";
                }
            } else {
                observation.nepaliDate = "";
    ...
    $scope.initializeNepaliDateTime = function(observation) {
        if (observation) {
            ...
            if (typeof observation.value === "string" && observation.value.trim()) {
                try {
                    var dateParts = observation.value.split(" ")[0];
                    if (dateParts) {
                        ...
                        if (date.length === 3 && date.every(part => part.trim())) {
                            ...
                            if (!isNaN(adDateObj.year) && !isNaN(adDateObj.month) && !isNaN(adDateObj.day)) {
                                ...
                                if (NepaliFunctions.BS.ValidateDate(bsDate)) {
                                    var formatted = NepaliFunctions.ConvertToDateFormat(bsDate, "YYYY-MM-DD");
                                    observation.nepaliDate = formatted;
                                    var dateObj = moment(observation.value).toDate();
                                    if (!isNaN(dateObj)) {
                                        observation.selectedTime = dateObj;
                                        observation.selectedDate = dateObj;
                                    } else {
                                        observation.selectedTime = null;
                                        observation.selectedDate = null;
                                    }
                                } else {
                                    observation.nepaliDate = "";
                    ...
                    console.error("Error initializing BS datetime:", e);
                    observation.nepaliDate = "";
                    observation.selectedTime = null;
                    observation.selectedDate = null;
                }
    ...
    $scope.clearNepaliDate = function() {
        if ($scope.observation) {
            ...
            $scope.observation.selectedDate = null;
            $scope.observation.selectedTime = null;
        }
    };

    var valueCompletelyFilled = function(observation) {
        return observation && observation.selectedDate instanceof Date && !isNaN(observation.selectedDate) && observation.selectedTime instanceof Date && !isNaN(observation.selectedTime);
    };

    var getSelectedDateStr = function(selectedDate) {
        return selectedDate instanceof Date && !isNaN(selectedDate) ? moment(selectedDate).format("YYYY-MM-DD") : "";
    };

    var getSelectedTimeStr = function(selectedTime) {
        return selectedTime instanceof Date && !isNaN(selectedTime) ? moment(selectedTime).format("HH:mm") : "";
    };

    var valueNotFilled = function(observation) {
        return observation && !observation.value && !observation.selectedTime;
    };

    $scope.isValid = function(observation) {
        return observation && (valueNotFilled(observation) || valueCompletelyFilled(observation));
    };
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.UIControls = Bahmni.Common.UIControls || {}, Bahmni.Common.UIControls.ProgramManagement = Bahmni.Common.UIControls.ProgramManagement || {}, angular.module("bahmni.common.uicontrols.programmanagment", []), angular.module("bahmni.common.uicontrols.programmanagment").controller("ManageProgramController", ["$scope", "retrospectiveEntryService", "$window", "programService", "spinner", "messagingService", "$stateParams", "$q", "confirmBox", "appService", function($scope, retrospectiveEntryService, $window, programService, spinner, messagingService, $stateParams, $q, confirmBox, appService) {
        ...
        // },
        $scope.updateAdDate = function(programEnrollmentDateBs) {
            if (programEnrollmentDateBs) {
                var dateStr = programEnrollmentDateBs.split("-"),
                    dateAD = NepaliFunctions.BS2AD({
                ...
                $scope.programEnrollmentDate = dateAD ? new Date(dateAD.year, dateAD.month - 1, dateAD.day) : null;
            } else {
...
Bahmni.Clinical.DispostionActionMapper = function() {
        ...
        $scope.handleNepaliDateUpdate = function(consultation) {
            ...
            if (conditionDateNepali) {
                ...
                if (dateAD) {
                    consultation.condition.onSetDate = new Date(dateAD.year, dateAD.month - 1, dateAD.day);
                } else {
                    consultation.condition.onSetDate = null;
                }
    ...
    angular.module("bahmni.clinical").controller("InvestigationController", ["$scope", "labTestsProvider", "otherTestsProvider", function($scope, labTestsProvider, otherTestsProvider) {
        ...
        var convertBsToAd = function(nepaliDate) {
            if (nepaliDate) {
                var dateStr = NepaliFunctions.ConvertToNumber(nepaliDate);
                var parts = dateStr.split("-");
                var year = parseInt(parts[0]);
                if (year < 100) {
                    year += 2000; 
