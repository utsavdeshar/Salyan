...
FROM
    (SELECT 'Pelvic inflammatory disease (PID)' AS answer_name 
     UNION SELECT 'Prolapsed uterus'
     UNION SELECT 'Menstrual disorder'
     UNION SELECT 'Dysfunctional Uterine Bleeding (DUB)'
     UNION SELECT 'Sub- fertility (Female)'
     UNION SELECT 'Sub- fertility/ infertility (Male)') first_answers
    LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE hdm.hmis IN (
        'Pelvic inflammatory disease (PID)', 'Prolapsed uterus', 
        'Menstrual disorder', 'Dysfunctional Uterine Bleeding (DUB)', 
        'Sub- fertility (Female)', 'Sub- fertility/ infertility (Male)')) raw_data_final 
...
ORDER BY FIELD(first_answers.answer_name,  
    'Pelvic inflammatory disease (PID)', 'Prolapsed uterus', 
    'Menstrual disorder', 'Dysfunctional Uterine Bleeding (DUB)', 
    'Sub- fertility (Female)', 'Sub- fertility/ infertility (Male)');
