...
FROM
    (SELECT 'Conjunctivitis' AS answer_name 
     ...
     UNION SELECT 'Blepharitis'
     UNION SELECT 'Neonatal Conjuctive Syndrome (NCS)') first_answers
    LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE hdm.hmis IN (
        ...
        'Retinoblastoma', 'Red eye', 'Blepharitis','Neonatal Conjuctive Syndrome (NCS)')) raw_data_final 
...
ORDER BY FIELD(first_answers.answer_name,  
    ...
    'Macular degeneration (age related)', 'Amblyopia (lazy eye)', 
    'Squint', 'Retinitis pigmentosa', 'Night blindness/visual disturbance', 
    'Retinoblastoma', 'Red eye', 'Blepharitis','Neonatal Conjuctive Syndrome (NCS)');
