...
FROM
    (SELECT 'Dementia' AS answer_name 
     UNION SELECT 'Addiction'
     UNION SELECT 'Schizophrenia'
     UNION SELECT 'Acute Psychotic Disorder'
     UNION SELECT 'Bipolar Affective Disorder'
     UNION SELECT 'Depression'
     UNION SELECT 'Phobic anxiety'
     UNION SELECT 'Psychosis'
     UNION SELECT 'Obsessive-compulsive disorder'
     UNION SELECT 'Conversive disorder (Hysteria)'
     UNION SELECT 'Neurosis'
     UNION SELECT 'Intellectual disability'
     UNION SELECT 'Epilepsy'
     UNION SELECT 'Migraine'
     UNION SELECT 'Mental illness (unspecified)'
     UNION SELECT 'Anxiety disorder'
     UNION SELECT 'Alcohol Use Disorder'
     UNION SELECT 'Substance use disorder') first_answers
    LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE hdm.hmis IN (
        'Dementia','Addiction', 'Schizophrenia', 'Acute Psychotic Disorder', 
        'Bipolar Affective Disorder', 'Depression', 'Phobic anxiety', 'Psychosis',
        'Obsessive-compulsive disorder', 'Conversive disorder (Hysteria)', 'Neurosis',
        'Intellectual disability', 'Epilepsy', 'Migraine', 
        'Mental illness (unspecified)', 'Anxiety disorder', 
        'Alcohol Use Disorder', 'Substance use disorder')) raw_data_final 
