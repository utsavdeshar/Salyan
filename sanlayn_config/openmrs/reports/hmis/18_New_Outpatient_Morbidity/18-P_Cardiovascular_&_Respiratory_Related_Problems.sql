...
FROM
    (SELECT 'Hypertension' AS answer_name 
     UNION SELECT 'Congestive heart failure'
     UNION SELECT 'Cardiac heart failure'
     UNION SELECT 'Chronic Obstructive Pulmonary Disease (COPD)'
     UNION SELECT 'Acute rheumatic fever'
     UNION SELECT 'Rheumatic heart disease (RHD)'
     UNION SELECT 'Ischemic heart disease'
     UNION SELECT 'Other cardiovascular problems'
     UNION SELECT 'Bronchial asthma') first_answers
    LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE hdm.hmis IN (
        'Hypertension', 'Congestive heart failure', 'Cardiac heart failure', 
        'Chronic Obstructive Pulmonary Disease (COPD)', 
        'Acute rheumatic fever', 'Rheumatic heart disease (RHD)', 
        'Ischemic heart disease', 'Other cardiovascular problems', 
        'Bronchial asthma')) raw_data_final 
