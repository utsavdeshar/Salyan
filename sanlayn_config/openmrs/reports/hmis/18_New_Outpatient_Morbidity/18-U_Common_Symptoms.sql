...
FROM
    (SELECT 'General consideration on pain' AS answer_name 
     UNION SELECT 'Chest pain'
     UNION SELECT 'Breathlessness/ Shortness of Breath'
     UNION SELECT 'Cough'
     UNION SELECT 'Abdominal pain'
     UNION SELECT 'Nausea and vomiting'
     UNION SELECT 'Headache'
     UNION SELECT 'Fever'
     UNION SELECT 'Dizziness and vertigo'
     UNION SELECT 'Syncope (Sudden Collapse or LOC)'
     UNION SELECT 'Itching (Pruritus)'
     UNION SELECT 'Fatigue and Weakness (Generalized)') first_answers
    LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE hdm.hmis IN (
        'General consideration on pain', 'Chest pain', 
        'Breathlessness/ Shortness of Breath', 'Cough', 
        'Abdominal pain', 'Nausea and vomiting', 'Headache', 
        'Fever', 'Dizziness and vertigo', 
        'Syncope (Sudden Collapse or LOC)', 'Itching (Pruritus)', 
        'Fatigue and Weakness (Generalized)')) raw_data_final 
