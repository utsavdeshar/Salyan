...
FROM
    (SELECT 'Leprosy' AS answer_name UNION SELECT 'Meningitis' AS answer_name ) first_answers
        LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE
        hdm.hmis IN ('Leprosy','Meningitis')) raw_data_final ON first_answers.answer_name = raw_data_final.hmis
