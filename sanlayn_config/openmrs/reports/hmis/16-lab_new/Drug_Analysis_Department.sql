...
from 
  ...
  left join (
    ...
    WHERE 
      ts.name = 'Drug Analysis Department' 
    GROUP BY 
      ts.name, 
      t.name, 
      t.id
...
where 
  lmt.department = 'Drug Analysis Department' 
  and lmt.is_active = 'Y' 
group by 
  lmt.department, 
  lmt.dhis2_test_name 
order by 
  CASE when lmt.dhis2_test_name = 'CARBAMAZEPINE' THEN 1 when lmt.dhis2_test_name = 'CYCLOSPORINE' THEN 2 when lmt.dhis2_test_name = 'VALPORIC ACID' THEN 3 when lmt.dhis2_test_name = 'PHENYTOIN' THEN 4 when lmt.dhis2_test_name = 'DIGOXINE' THEN 5 when lmt.dhis2_test_name = 'TACROLIMUS' THEN 6  END
