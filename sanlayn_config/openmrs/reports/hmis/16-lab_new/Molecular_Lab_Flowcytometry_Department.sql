...
from
   ...
   left join
      (
         ...
         WHERE
            ts.name = 'Molecular Lab & Flowcytometry Department' 
         GROUP BY
            ts.name, t.name, t.id 
      )
      raw_data 
...
where
   lmt.department = 'Molecular Lab & Flowcytometry Department' 
   and lmt.is_active = 'Y' 
group by
...
order by
   CASE
      when
         lmt.dhis2_test_name = 'COVID-19 PCR' 
      THEN
         1 
      when
         lmt.dhis2_test_name = 'INFLUENZA' 
      THEN
         2 
      when
         lmt.dhis2_test_name = 'LEUKEMIA PANEL' 
      THEN
         3 
   END